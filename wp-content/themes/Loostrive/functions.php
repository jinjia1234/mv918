...
global $commentcount,$wpdb, $post;
     if(!$commentcount) { //初始化楼层计数器
          ...
          $page = get_query_var('cpage',0);//获取当前评论列表页码
	...
	if (get_option('strive_timth')=='Display'){
		if ( has_post_thumbnail() ) {
            ...
            if ($n > 0) {
                ...
                $parse_url = parse_url(str_replace('.doubanio.com','',$strResult[1][0]));
                echo '<img src="' . get_bloginfo("url") . '/timthumb_' . $width . '_' . $height . '/' . $parse_url['host'] . $parse_url['path'] . '" width="' . $width . '" height="' . $height . '" alt="' . $title . '" />';
            } else {
	...
	if (get_option('strive_timth')=='Display'){
		if ( has_post_thumbnail() ) {
			...
			if($n > 0){
                ...
                $parse_url = parse_url(str_replace('.doubanio.com','',$strResult[1][0]));
                echo '<img src="' . get_bloginfo("url").'/timthumb_'.$width.'/' . $parse_url['host'] . $parse_url['path'] . '" alt="'.$title.'" />';
			}
		}
	}else{
		if ( has_post_thumbnail() ) {
